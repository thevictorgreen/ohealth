...
define('MARTIN_URL', "http://upload.wikimedia.org/wikipedia/en/6/61/I_Have_A_Dream_sample.ogg");
...
abstract class KLTest extends PHPUnit_Framework_TestCase {
    ...
    public function setUp(){
        $this->CONFIG = array();
        $this->CONFIG['API_HOST'] = "http://192.168.1.169:8780";
...
class WrapperTest extends KLTest
{
    ...
    public function testCreateSpeakerModel() {
        $response = $this->api->create_speaker_model(array(MARTIN_URL), null, "martin luther");
        $this->assertObjectHasAttribute("model_id", $response);
        $this->assertObjectHasAttribute("modality", $response);
        $this->assertEquals("martin luther", $response->name);
    ...
    public function testRecognizeFace() {
        ...
        $this->assertObjectHasAttribute("faces", $response);
        $this->assertGreaterThan(90, $response->faces[0]->results[0]->score);

        # Test with image of another person (get lower score)
        $response = $this->api->recognize_face(array($model_id), array(LENA_URL));
        $this->assertObjectHasAttribute("faces", $response);
        $this->assertLessThan(90, $response->faces[0]->results[0]->score);
        ...
        $this->assertGreaterThan(90, $response->models[0]->score);
    }
    ...
    public function testRecognizeSpeaker() {
        ...
        $response = $this->api->recognize_speaker(array($model_id), null, array(MARTIN_URL), null, null, FALSE);
        $this->assertObjectHasAttribute("audios", $response);
        $this->assertGreaterThan(90, $response->audios[0]->results[0]->score);

        $response = $this->api->recognize_speaker(array($model_id), null, array(ARMSTRONG_URL), null, null, FALSE);
        $this->assertObjectHasAttribute("audios", $response);
        $this->assertLessThan(90, $response->audios[0]->results[0]->score);
    ...
    public function testStream(){
        ...
        $lena_image_data = array($this->download_url(LENA_URL));
        for ($i = 0; $i < 3; $i++){
    ...
    public function testCreateModelShortcutIdentity() {
        ...
        $response = $this->api->create_identity(null, array(MARTIN_URL), null, "speaker identity", "speaker");
        $this->assertObjectHasAttribute("identity_id", $response);
        $this->assertObjectHasAttribute("nb_models", $response);
        $this->assertEquals(1, $response->nb_models);
    }
